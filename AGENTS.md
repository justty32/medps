...
- 主要 build 指令：`cmake -S . -B build && cmake --build build`
- 主要 test 指令：`./build/bin/medp_test.<平台>.<組態>.<位元數>`（如 `medp_test.linux.debug.64`），詳見 [workflows/testing.md](workflows/testing.md)
...
- 人要重讀程式碼 → [CODE_TOUR](workflows/common/code-map/CODE_TOUR.md)（線性導讀）；想快速回顧進度 → [work/progress_overview.md](work/progress_overview.md)。
- 既有設計/分析文檔 → `work/`（design、architecture）與 `notes/`（設計草稿，非源碼）。
...
- **新增 component 時必須同步登記 `src/gcore/serialize/all_components.h` 的 `AllComponents`**，否則存檔會漏掉它。
...
- `include/` 是第三方 header-only 庫（entt、cereal），不要修改。
- `build/` 是產出目錄，不 commit。
- 歷史分析/設計文檔在 `work/`；新的調查、設計、計畫走對應工作流（investigation / specs / plans），既有 `work/` 文檔先保留、用連結引用，不強制搬遷。
