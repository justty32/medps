...
- **待親自驗證** 策略遊戲可玩原型 (`projects/game/`) — 建置成功，執行檔 `projects/game/build/bin/medp_game.windows.debug.64.exe`。建置指令：`cmake -S projects/game -B projects/game/build -G "MinGW Makefiles" && cmake --build projects/game/build`。**功能**：80×40 ASCII 世界地圖（FBM 生成），玩家 2 城 3 部隊 vs AI 2 城 3 部隊，回合制，佔領所有敵城為勝；控制：Enter 選取，方向鍵移動，S 略過，T 結束回合，Q 離開。**需親自執行驗證**：(1) 世界地圖能正確顯示地形色彩；(2) 部隊可選取/移動/戰鬥；(3) AI 回合後能繼續玩家回合；(4) 勝利/失敗畫面出現。
